...
- Re-ran screen + validate off the existing cache (no rebuild, no drive needed) to regenerate the CSVs under the honest rules; the pass count drops sharply from 476 (results in the new `validation_results` CSV).
