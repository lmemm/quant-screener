...
## [2026-06-05] — First real run + T-008 honest validation

- Ran the full pipeline on the real drive (off the cache): 19,285 tickers, 3,479 cleared the share-volume filter, and validation passed **476/1,053 (~45%)** — way too many. Verified the cause: it was mostly **bull-market beta**, not edge. Passers were positive buy-and-hold 65% of the time vs 36% for non-passers; the pass bar was merely "any positive return"; and illiquid microcaps showed fantastical fold returns (up to +448%) under an unrealistic flat 0.1% cost. The benchmark again did its job — it stopped a bogus list from looking like signal.
- T-008 makes validation honest: a fold passes only if the strategy **beats buy-and-hold** that fold (excess > 0); gate on **dollar volume** (`MIN_DOLLAR_VOLUME` $5M) not share count (`characterize` now reports `avg_dollar_volume`); charge **liquidity-aware costs** (`WFV_COST_TIERS`, 5/15/30 bps) instead of a flat fee. Output reports `mean_buy_hold`/`mean_excess_return` and ranks by excess. The owner explicitly rejected an arbitrary *price* floor — correct: the lever is liquidity + realistic cost, not excluding cheap names (logged in DECISIONS).
- Caught a test-isolation regression this surfaced: once a real cache exists at the default path, `screen_universe(sample_drive)` auto-used it and screened all 19k (a 7-minute "unit" test). Sample-drive screen tests now pass an explicit empty `cache_dir`. `ruff` clean, `pytest` 69 passed.
- Filed follow-ups: T-009 (Hurst label skew, 1,020 Trending vs 33 Mean-Reverting — likely R/S upward bias) and T-010 (16 cache filename collisions from `/`→`_`).
- Re-ran screen + validate off the existing cache (no rebuild, no drive needed). First honest re-run *raised* the pass rate to 54.6% (428/784) — the re-run caught a flaw in the new criterion: requiring "beat buy-and-hold" alone lets a long-only strategy pass on stocks that **crashed** (it sat in cash and "beat" the slide, e.g. TSP −1.7% vs buy-hold −48%) without making money. Corrected the gate to require **both** profitable **and** beats buy-and-hold; added a test for the crash-avoider case. The dollar-volume gate also trimmed 1,053 → 784 evaluated (dropped 269 illiquid names). Re-ran again under the corrected rule.
- Corrected re-run: **83/784 passed (10.6%)** — a believable filter result (Trending 63, Mean-Reverting 20). 70 of 83 are net-positive overall; 13 (16%) passed on ≥3 of 6 folds yet have a *negative* overall mean (won in half the periods, lost more in the rest). Only 2 names cleared ≥5/6 folds. Filed T-011 to consider a stricter overall gate (net-positive and/or ≥4 folds). The list is still breakout-dominated, echoing the Hurst skew (T-009).
- Next: review the corrected `validation_results` CSV; work T-009 (Hurst skew), T-011 (stricter overall gate), T-010 (cache collisions).

## [2026-06-06] — T-009 Hurst estimator bias

- Confirmed the suspected skew was an estimator artifact, not signal. Built a diagnostic (`scratch/hurst_bias.py`) that synthesizes exact fractional Gaussian noise of *known* H via Cholesky and runs the current estimator on it: the R/S method was biased **high by +0.05 to +0.08**, worst on short series. The smoking gun — a true random walk (H=0.50) was labelled **Trending 46–62%** of the time and Mean-Reverting essentially never. That fully explains the 31:1 screen skew (and why only 33 names ever landed Mean-Reverting).
- Fixed with the **Anis-Lloyd-Peters correction** (`_expected_rs` + `H = 0.5 + empirical_slope − expected_iid_slope`). The same diagnostic on the corrected estimator: true random walk → ~0.47–0.48, Trending false-positive rate down to 0–6%. Kept the R/S method rather than swapping to DFA — the bias had a known, principled correction.
- The correction **flipped the real-universe split** toward Mean-Reverting (mean Hurst 0.452), which was the genuinely instructive part. Two lessons: (1) daily single-stock returns really are mildly mean-reverting — short-term reversal is real, and equity *momentum* is cross-sectional/multi-month, not a daily-autocorr effect, so the old "everything trends" was the artifact; (2) a small residual negative bias remains and the old 0.45/0.55 thresholds are tighter than the estimator's ~±0.05 per-series noise. Surfaced the fork to the owner and (on their ask) recommended + applied **threshold recalibration to ground truth**: `HURST_TREND_MIN 0.55→0.51`, `HURST_REVERT_MAX 0.45→0.43`, so a true-0.55 series lands Trending and true-0.45 lands Mean-Reverting. Chose this over a noise-margin band (which nuked Trending to 2 names) or keeping the un-calibrated thresholds (47% Mean-Reverting, a mirror-image artifact).
- Real-universe split corrected: **1,020/33/2,426 → 158 Trending / 856 Mean-Reverting / 2,465 Random** (4.5% / 24.6% / 70.9%) over 3,479 candidates. The residual per-name label noise is left for walk-forward validation to absorb (wrong strategy → fails out-of-sample → rejected, not a false positive). Confirmed on real data off the cache (two ~5-min screen re-runs, no drive/rebuild needed). 4 regression tests added; `ruff` clean, `pytest` 74 passed. Decision + caveats in DECISIONS.md.
- Re-ran **validation** on the corrected labels: **311/756 passed (41%)**, dominated by Mean-Reverting (300). That high rate + implausible top excess returns (VRPX +530%, ISPC +553%, VIXY +102%, SCO +60%) is a **red flag, not a win** — the corrected labels exposed that the universe is contaminated with leveraged/inverse/volatility ETPs (VIXY, SCO) and explosive microcaps the dollar-volume gate doesn't catch. T-009 itself is sound (label fix verified against synthetic ground truth, independent of the pass rate); this is a *separate downstream* issue. Opened **PR #8** (mergeable, clean, 74 tests green) for the T-009 code change.
- Filed the finding so it isn't lost: new **T-012** (universe hygiene — exclude ETPs/leveraged/inverse/volatility instruments) and strengthened **T-011**'s motivation (stricter overall gate, best done after T-012 cleans the universe). Annotated T-010/T-011/T-012 acceptance criteria with `[local-deferred]` markers so a web session isn't blocked on the cache-backed confirmation runs.
- **Output trustworthiness:** `screen_results_2026-06-06.csv` is good (corrected labels). `validation_results_2026-06-06.csv` is **not yet trustworthy for promotion** — it reflects the contaminated universe; wait for T-012 + T-011 before promoting names.
- Next (open): **PR #8 awaiting merge**; then T-012 (universe hygiene, P2), T-011 (stricter gate, P2), T-010 (cache filename collisions, P3). A final clean validation run after T-012+T-011 produces the first promotable pass list.
